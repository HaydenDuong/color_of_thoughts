...
- **Wall motion (`/wall`):** every sphere moves with simple 2D billiard physics — elastic collisions between spheres and against the camera's visible rectangle, 98% restitution, all-elastic pairwise impulses. **Turbulence-aware:** the tank is split into three horizontal bands (calm 1–2 at the bottom, mixed 3 in the middle, turbulent 4–5 at the top). Each sphere's `maxSpeed`, per-axis ambient jitter, vertical home-spring, and extra vertical damping are looked up from its rating, so calm spheres flow mostly horizontally while turbulent ones bounce frantically. A soft spring pulls each sphere back toward its band home after cross-band collisions, and initial X/Y are seeded from `participant_id` + the rating's band so spawns never stack. The whole group also parallaxes toward the mouse (Codrops-style); on the exhibition machine with no mouse input it simply sits still.
