...
- **Wall motion (`/wall`):** every sphere moves with 2D billiard physics — elastic collisions between spheres and against the camera's visible rectangle, 98% restitution, all-elastic pairwise impulses. The wall now has **two primary motion modes** (plus one hidden comparison mode), toggled with a pill switch in the top-right of `/wall`; the choice is mirrored into the URL (`?mode=...`) so two tabs can show the modes side-by-side and a refresh keeps the current view.
  ...
  - Switching modes is **state-preserving**: the same `PhysicsSphere[]` is reused, only the step function swaps. Positions and velocities carry over and the new mode's forces guide the ensemble into its new layout over ~1–2 s with no teleport.
  - The whole group also parallaxes toward the mouse (Codrops-style); on the exhibition machine with no mouse input it simply sits still.
