...
| 2026-04-26 | **Wall background v2 — CodePen-style visible gradient flow**: v1 looked colorful but effectively still on the wall because tiny radial-gradient drifts were too subtle behind the 3D scene. Kept the transparent WebGL canvas, but changed `.wall-canvas-wrap::before` to the CodePen-style motion model from `P1N2O/pyBNzX`: one large `linear-gradient(-45deg, ...)` with `background-size: 420% 420%`, animated from `background-position: 0% 50% → 100% 50% → 0% 50%` over 22 s. This makes the color field visibly travel across the wall from a distance. The `::after` layer is now only a subtle soft-light cloudy veil (low-opacity radial blobs + light wash) so the background keeps some watercolor texture without hiding the main movement. `prefers-reduced-motion: reduce` still freezes the CSS animation. |
| 2026-04-26 | **Wall background v1 — animated aurora gradient**: replaced the wall canvas's flat cream WebGL clear color with a transparent Canvas (`gl.alpha=true`, removed `<color attach="background">`) so CSS can render behind the 3D scene. Added a wall-only layered CSS background on `.wall-canvas-wrap`: multiple soft `radial-gradient()` color clouds (cyan/green/yellow/coral/rose/magenta) over a slow `linear-gradient()` drift, plus a translucent cream/white veil so the projection stays soft and the colorful blobs remain readable. Motion is deliberately slow (`42s` aurora drift, `58s` veil drift) to match the psychology-exhibition mood rather than compete with Mandala meteor motion. `prefers-reduced-motion: reduce` freezes the CSS animation. Applies behind every wall mode without touching physics or shader code. |
| 2026-04-26 | **Mandala tuning v6 — calm cover rotates with scaffold**: scaffold already rotated visually, but calm/docked user blobs were springing to static world-space dock slots, so the navy Fibonacci sphere rotated *under* the calm layer instead of the calm layer feeling attached to the mandala. Kept user blobs in world-space physics (important for meteor collisions) but now pass the scaffold's current Y-rotation angle from `MandalaStage` into `stepMandalaPhysics`. Docked blobs rotate by chasing `dockPositions[dockIdx]` transformed by that same Y angle; knocked-loose calm blobs (`dockIdx=null` during grace) and chaotic meteors remain independent world-space objects. Also added tangential-velocity feed-forward for dock targets: damping is relative to the rotating slot's velocity, not zero world velocity, so docked calm blobs ride the rotating surface instead of visibly trailing their slots. This is Option B from the discussion: calm blobs rotate only while truly docked; when hit, they peel away, drift, then reclaim a new rotating slot. |
